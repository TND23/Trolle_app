TrolleApp.Views.BoardShow = Backbone.View.extend({
  ...
  createCard: function(event){
    event.preventDefault();
		...
		var newCard = new TrolleApp.Models.Card({list_id: current_list.id});
		TrolleApp.card = newCard;
				newCard.save(newCard.attributes,
			{
				...
				error: function(model, response){console.log(model); console.log(response);}
			});
  ...
  createList: function(event){
	...
	destroyCard: function(event){
		event.preventDefault();
		var list_id = parseInt(event.target.parentElement.parentElement.id.match(/\d+/));

		var target_card_id = this.getTarget(event);
		var card = TrolleApp.boardLists.get(target_card_id);
		var cardHolder = $("#cardHolder"+target_card_id);
  ...
  showListInfo: function(event){
	...
	renderList: function(id){
		...
		$('.lists').append(JST['lists/show']({collection: this.collection, board: this.board, current_list: list_of_models[current_index]}));
		this.makeDraggable();
		this.makeDroppable();
	},
